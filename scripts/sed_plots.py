...
all_matched = pd.read_csv("../psrc_lists/final_matched_snr_5_3arcmin_snr_7.csv")
f_all_matched = all_matched.loc[(all_matched["theta_1"]==2.0)&(all_matched["theta_2"]==6.0)]

freqs = [3e9,1.4e9,76e6,84e6,92e6,99e6,107e6,115e6,122e6,130e6,143e6,151e6,158e6,166e6,174e6,181e6,189e6,197e6,204e6,212e6,220e6,150e6,2.4275e14,1.8038e14,1.3886e14,8.901e13,6.492e13,2.4813e13,1.3508e13]
...
for i in range(len(f_all_matched)):
    ...
    if np.sum(np.isnan(cat_fluxes_fit))<2:
        ...
        p = np.repeat(999,2)
    row=np.append(np.array(list(f_all_matched.iloc[i][["cluster","ra_deg","dec_deg","FINT_first","Total_flux_vlass","peak_flux_mJy_adj"]])),p)
...
df_sed.to_csv("../psrc_lists/spectral_indices.csv")
